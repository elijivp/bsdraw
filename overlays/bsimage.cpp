...
OImageOriginal::OImageOriginal(QImage* image, IMAGECONVERT icvt, COORDINATION cn, float x, float y, float mult_w, float mult_h, float anch_lr01, float anch_bt01): 
  Ovldraw_Image(image, icvt, g_bs_detach_image), OVLCoordsDynamic(cn, x, y), OVLDimms2Dynamic(CR_ABSOLUTE_NOSCALED, 0.0f, 0.0f),
  m_anch_lr01(anch_lr01), m_anch_bt01(anch_bt01)
{
...
OImageOriginal::OImageOriginal(QImage *image, IMAGECONVERT icvt, OVLCoordsStatic *pcoords, float offset_x, float offset_y, float mult_w, float mult_h, float anch_lr01, float anch_bt01):
...
int OImageOriginal::fshOVCoords(int overlay, bool switchedab, char *to) const
{
  ...
  {
    ocg.var_const_fixed("anchor", m_anch_lr01, m_anch_bt01);
    
    ocg.goto_normed();
    if (/*m_anchor == 10*/true)
    {
      ocg.push("inormed = inormed + ivec2(idimms2*anchor);");
